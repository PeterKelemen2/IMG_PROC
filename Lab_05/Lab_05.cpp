...
void performAdaptiveThreshold() { // könyv maszkolása?
	cv::Mat I = cv::imread("text.png",0); // fontos, hogy 0-át odaírjunk
	cv::Mat thresholded;
	cv::Mat manualthresholded, manualmasked;

	int blockSize = 5;
	cv::adaptiveThreshold(I, thresholded, 255, cv::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY, blockSize, 2);

	cv::threshold(I, manualthresholded, 120, 255, cv::THRESH_BINARY);
	cv::bitwise_and(I, manualthresholded, manualmasked);
	
	cv::imshow("adaptive", thresholded);
	cv::imshow("manual", manualthresholded);
	cv::waitKey();
}

void performManualThreshold() { // maszkolás
	cv::Mat I = cv::imread("rose.jpg", 0);
	cv::Mat thresholded;
	cv::Mat masked;
	unsigned char thresh_value = 100;

	cv::threshold(I, thresholded, thresh_value, 255, cv::THRESH_BINARY);

	cv::imshow("rose mask",thresholded);
	cv::waitKey();


	cv::bitwise_and(I, thresholded, masked);

	cv::imshow("rose masked", masked);
	cv::waitKey();
}
